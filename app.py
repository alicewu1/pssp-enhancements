...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+mysqldb://' + AZURE_MYSQL_USERNAME + ':' + AZURE_MYSQL_PASSWORD + '@' + AZURE_MYSQL_HOSTNAME + ':3306/patient_portal'
...
class Procedures_patient(db.Model):
    __tablename__ = 'production_patient_treatment_procedures'

    id = db.Column(db.Integer, primary_key=True)
    mrn = db.Column(db.String(255), db.ForeignKey('production_patients.mrn'))
    CPT_code = db.Column(db.String(255), db.ForeignKey('production_treatment_procedures.CPT_code'))
...
class Procedures(db.Model):
    __tablename__ = 'production_treatment_procedures'
...
def insert(): # note this function needs to match name in html form action 
    if request.method == 'POST':
        ...
        contact_mobile = request.form['contact_mobile']
        contact_home = request.form['contact_home']
        new_patient = Patients(mrn, first_name, last_name, dob, zip_code, gender, contact_mobile, contact_home)
        db.session.add(new_patient)
        db.session.commit()
...
# Endpoint for Adding A New Medication To A Patient
...
def update_medications(): # note this function needs to match name in html form action
    if request.method == 'POST':
        ...
        patient_medications.med_ndc = request.form.get('med_ndc')
        db.session.commit()
        flash("Patient Medications Updated Successfully")
        ## then return to patient details page
